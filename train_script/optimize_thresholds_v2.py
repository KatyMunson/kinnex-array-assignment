...
classification. Extends optimize_thresholds.py with a grid search over:
  ...
  - minimum specific barcode observations per ZMW (optional)
...
Usage:
    ...
    # Tighter accuracy target, skip specific sweep for speed:
    python optimize_thresholds_v2.py \\
        --assignments results/assigned/*.txt \\
        --lookups results/lookup/*_reads.txt \\
    ...
    for assign_file in assignment_files:
        ...
        for _, row in df.iterrows():
            ...
            all_rows.append({
                ...
                'n_specific':    row.get('Specific_Barcodes', np.nan),
                'n_shared':      row.get('Shared_Barcodes', np.nan),
                'n_discordant':  row.get('Discordant_Barcodes', np.nan),
...
def grid_search(df, posterior_thresholds, min_obs_values, min_specific_values):
    """
    Sweep all combinations of (posterior, min_obs, min_specific).

    Returns a DataFrame with one row per combination containing:
        posterior, min_obs, min_specific, n_kept, pct_kept, n_correct,
        n_errors, accuracy
    """
    n_total = len(df)
    records = []

    total_combos = len(posterior_thresholds) * len(min_obs_values) * len(min_specific_values)
    print(f"\nRunning {total_combos:,} threshold combinations "
          f"({len(posterior_thresholds)} posterior × "
          f"{len(min_obs_values)} min_obs × "
          f"{len(min_specific_values)} min_specific)...")

    for post, min_obs, min_specific in product(posterior_thresholds, min_obs_values, min_specific_values):
        mask = (df['posterior'] >= post)
        if min_obs > 0:
            mask &= (df['n_obs'] >= min_obs)
        if min_specific > 0:
            mask &= (df['n_specific'] >= min_specific)
...
def recommend(grid_df, target_accuracy, label="HIGH_CONF"):
    ...
    print(f"  Min specific ≥ {int(best['min_specific'])}")
...
def save_numerical_tables(grid_df, pareto_df, recommendations, output_prefix):
    ...
    display_cols = ['posterior', 'min_obs', 'min_specific',
    ...
    for label, rec in recommendations.items():
        rec_rows.append({
            ...
            'min_specific': int(rec.get('min_specific', 0)),
...
def plot_1d_sweeps(df, grid_df, recommendations, output_prefix):
    ...
    computed at the recommended min_obs / min_specific gate from the 2D search.
    Also adds a posterior distribution panel.
    """
    rec = recommendations['HIGH_CONF']
    min_obs_fixed      = int(rec['min_obs'])
    min_specific_fixed = int(rec['min_specific'])
    ...
    for p in post_vals:
        ...
        if min_specific_fixed > 0:
            mask &= (df['n_specific'] >= min_specific_fixed)
        kept = df[mask]
        if len(kept) == 0:
            continue
        rows.append({
    ...
    fig.suptitle(
        f"1D Posterior Sweep  (min_obs ≥ {min_obs_fixed}, min_specific ≥ {min_specific_fixed})",
...
def plot_heatmaps(grid_df, recommendations, output_prefix, row_dim='min_obs'):
    ...
    row_dim : str
        ...
        'min_obs'      → y-axis = min_obs,      facets iterate over min_specific values
        'min_specific' → y-axis = min_specific, facets iterate over min_obs values
    ...
    facet_dim  = 'min_specific' if row_dim == 'min_obs' else 'min_obs'
...
def plot_pareto(pareto_df, all_grid_df, recommendations, output_prefix):
    ...
    for ax, colour_by, label in [
        ...
        ax.scatter(
            ...
            label=f"Recommended\n(p≥{rec['posterior']:.3f}, "
                  f"obs≥{int(rec['min_obs'])}, specific≥{int(rec['min_specific'])})"
        )
...
def plot_obs_marginals(df, output_prefix):
    """
    Show how accuracy and ZMW counts vary with n_obs and n_specific independently,
    to give intuition for where the hard gates should sit.
    """
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("Marginal Analysis: Observations and Specific Barcodes",
                 fontsize=13, fontweight='bold')

    for row_i, (col, xlabel) in enumerate([
        ('n_obs',      'Total Segments (N_Observations)'),
        ('n_specific', 'Specific Barcodes'),
    ]):
...
def main():
    ...
    parser.add_argument('--max-specific', type=int, default=4,
                        help='Maximum min_specific gate to test (tests 0 … max-specific)')
    parser.add_argument('--no-specific-sweep', action='store_true',
                        help='Skip the min_specific dimension (only sweep posterior × min_obs)')
    ...
    min_specific_values = [0] if args.no_specific_sweep else list(range(0, args.max_specific + 1))

    # ---- Grid search ----
    grid_df = grid_search(df, posterior_thresholds, min_obs_values, min_specific_values)
    ...
    print(pareto_df[['posterior', 'min_obs', 'min_specific',
    ...
    # LOW_CONF: sweep posterior only (min_obs=0, min_specific=0) below the HC posterior
    lc_grid = grid_df[
        (grid_df['posterior'] < recommendations['HIGH_CONF']['posterior']) &
        (grid_df['min_obs']      == 0) &
        (grid_df['min_specific'] == 0)
    ]
    if len(lc_grid) > 0:
        recommendations['LOW_CONF'] = recommend(lc_grid, args.low_conf_target_accuracy, "LOW_CONF")
    else:
        recommendations['LOW_CONF'] = {'posterior': 0.5, 'min_obs': 0, 'min_specific': 0}
    ...
    current = {
        ...
        'min_specific': args.current_min_specific,
    }
    ...
    # Heatmap: y-axis = min_obs, facets = min_specific values → file: _heatmap_rowsmin_obs.png
    plot_heatmaps(grid_df, recommendations, output_prefix, row_dim='min_obs')

    # Heatmap: y-axis = min_specific, facets = min_obs values → file: _heatmap_rowsmin_specific.png
    if not args.no_specific_sweep and args.max_specific > 0:
        plot_heatmaps(grid_df, recommendations, output_prefix, row_dim='min_specific')
    ...
    output_data = {
        ...
        'sweep': {
            ...
            'min_specific_range': [0, 0 if args.no_specific_sweep else args.max_specific],
